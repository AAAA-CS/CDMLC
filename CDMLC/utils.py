...
PLT_FIGURE = plt.figure(frameon=False)
...
#遍历all_set中的每个类别，并检查每个类别的样本数量是否大于等于200。对于满足条件的类别，函数会将最后200个样本添加到名为all_good的新字典中，并统计满足条件的类别数和样本数。
...
def load_data(image_file, label_file):
    ...
    [nRow, nColumn, nBand] = data_all.shape     #paviaU 610 340 103
    print(data_key, nRow, nColumn, nBand)

    data = data_all.reshape(np.prod(data_all.shape[:2]), np.prod(data_all.shape[2:]))    #610×340=207400 103

    data_scaler = preprocessing.scale(data.astype(float))  # 610×340=207400 103 (X-X_mean)/X_std,
    Data_Band_Scaler = data_scaler.reshape(data_all.shape[0], data_all.shape[1], data_all.shape[2])     # 610 340 103
